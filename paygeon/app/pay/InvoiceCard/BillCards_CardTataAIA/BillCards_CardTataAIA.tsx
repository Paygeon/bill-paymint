...
export const BillCards_CardTataAIA: FC<Props> = memo(function BillCards_CardTataAIA({
  ...
  const modifyInvoiceId = (id: string) => {
    return id.split('-').map(part => part[0]).join('').toUpperCase();
  }

  useEffect(() => {
    const modifiedInvoiceId = modifyInvoiceId(invoiceData.invoice_id);
    setInvoiceId(modifiedInvoiceId);
  })
        ...
        <div className={classes.frame11208}>
          <div className={classes.frame11204}>
            ...
            <div className={classes.frame11205}>
              <div className={classes.iNV352456}>INV-{invoiceId}</div>
